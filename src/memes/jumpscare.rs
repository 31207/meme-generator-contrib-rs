...
const GIF_INFO: GifInfo = GifInfo {
    duration: 0.026,
    frame_num: 18,
};
...
fn jumpscare(images: Vec<InputImage>, _: Vec<String>, _: NoOptions) -> Result<Vec<u8>, Error> {
    let func = |i: usize, images: Vec<Image>| {
        ...
        if images[0].width() > 280 || images[0].height() > 280 {
            pic = images[0].resize_fit((280, 280), Fit::Contain);
        }
        ...
        let factor = 1.0 + double_heartbeat(i as f32 / GIF_INFO.frame_num as f32) * 2.0;
